...
//            pre = [NSPredicate predicateWithFormat:predicateString];
            pre = [NSPredicate predicateWithFormat:@"normalizedTitleWords.normalizedWord CONTAINS[cd] %@",searchString];

            NSLog(@"%@", pre);
            req.predicate = pre;
            req.sortDescriptors = [NSArray arrayWithObject:[NSSortDescriptor sortDescriptorWithKey:@"title" ascending:YES]];
        }
            break;
        case kSegmentedControlDescriptionIndex: {
        }
